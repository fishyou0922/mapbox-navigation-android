...
class ReplayRouteSmootherTest {
    ...
    fun `should turn straight road into a segment`() {
        ...
        assertEquals(37.758201, smoothedRoutes[0].point.latitude())
        assertEquals(-122.39338, smoothedRoutes[1].point.longitude())
        assertEquals(37.760208, smoothedRoutes[1].point.latitude())
    }

    @Test
    fun `should turn a turn into three points`() {
        // Geometry used anq_gAxdhmhFbZkA^?tDMUsF?m@WmKMoHOeF]eO?}@GiBcB}s@?{@McGoDLu@?cUlAqMj@qfAtE
        val coordinates = listOf<Point>(
    ...
    fun `should detect side of road`() {
        ...
        assertTrue(rightSideDistance!! < 0)
        assertTrue(leftSideDistance!! > 0)
    }
